...
Player::Player(float x , float y , float width , float height, color_t color , float speed)
{
    position.x = x;
    position.y = y;
    this->width = width;
    ...
    //setup rectangle and helmet vertices
    float verts[] = {
        // first triangle
...
{   
    ...
    color_buffer_helm[0] = COLOR_DARKWHITE.r;
    color_buffer_helm[1] = COLOR_DARKWHITE.g;
    color_buffer_helm[2] = COLOR_DARKWHITE.b;
    for(int i=3;i<(3*vao_helm->NumVertices);i++)
    {

        if(i%3 == 0)
            color_buffer_helm[i] = COLOR_GREY.r;
        else if(i%3 == 1)
            color_buffer_helm[i] = COLOR_GREY.g;
        else
            color_buffer_helm[i] = COLOR_GREY.b;
    }
