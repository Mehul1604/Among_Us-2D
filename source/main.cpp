...
#include "utilityHeaders/camera.h"
...
// game object
Game AmongUs(SCR_WIDTH , SCR_HEIGHT);
...
int main(int argc , char** argv){
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        // hud text update
        sprintf(health_str , "Health: %d" , AmongUs.player_health);
        sprintf(tasks_str , "Tasks done: %d/%d" , AmongUs.tasks_done , AmongUs.total_tasks);
        sprintf(time_str , "Time left: %d" , AmongUs.time_remaining);
        ...
        if(AmongUs.State == GAME_LOST || AmongUs.State == GAME_WON)
        {
            if(AmongUs.State == GAME_WON)
                draw_win();
            else
                draw_lose();
        }
